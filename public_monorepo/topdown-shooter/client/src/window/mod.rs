use sdl2::{keyboard::KeyboardState, mouse::MouseState};
use {
...
impl Window {
  ...
  pub fn update_mouse(&mut self) -> () {
    ...
    self.mouse.update(mouse_state);
  }

  pub fn update_keyboard(&mut self) -> () {
    let keyboard_state: KeyboardState = self.event_pump.keyboard_state();
    self.keyboard.update(keyboard_state);
  }
}
